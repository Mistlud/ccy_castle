...
Selecting an audio file opens an expanded player that uses the current folder's versioned 4:3 thumbnail. On desktop it is a compact bottom-right card; on mobile it becomes a full-height bottom sheet. The top handle collapses or restores it by click, and touch users can also swipe the handle down or up. The collapsed state is a slightly taller fixed bottom row with a square folder thumbnail, overflowing track name, muted folder-name subtitle, and compact previous/play-pause/next controls. Its balanced bottom and safe-area spacing prevents the controls from appearing pinned to the screen edge.
